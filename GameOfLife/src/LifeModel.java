	...
	private void oneGeneration(){
		...
		for (int r = 0; r < this.grid.length; r++){
			...
			for (int c = 0; c < this.grid[r].length; c++){
				sides = 0;
				//System.out.println(this.grid[r]);
				for (int x = -1; x <= 1; x++){
					for (int y = -1; y <= 1; y++){
						if ((x != 0 || y != 0) && x + r >= 0 && x + r < this.grid.length && y + c >= 0 && y + c < this.grid[r].length){
							//System.out.println(this.grid.length + ", " + this.grid[r].length);
							if (this.grid[r + x][c + y].isAliveNow()){
								sides++;
							}
						}
					}
				...
				if (!this.grid[r][c].isAliveNow()){
					if (sides == 3) {this.grid[r][c].setAliveNext(true);}
				}
				else if (this.grid[r][c].isAliveNow()){
					if (sides>= 2 && sides <= 3){this.grid[r][c].setAliveNext(true);}
					else{this.grid[r][c].setAliveNext(false);}
				}
